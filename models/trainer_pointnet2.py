...
def infer_model(dataset_iterator, model_path, net_out):
    with tf.Session() as sess:
        ...
        pred = sess.run([net_out], feed_dict = {pcl_placeholder : data[:,:,:3],
...
def calculate_accuracy(prediction, labels):
    ...
    correct = np.sum(np.equal(prediction, labels))
    return (correct/ (batch_size*len(prediction[0])))
...
def train(dataset_iterator, num_iteration, loss, pred):
    ...
    logging.basicConfig(level=logging.INFO, filename="logs/pointnet2_2.txt")
    ...
    with tf.Session() as sess:
        ...
        while(iter < num_iteration):
            _, batch_loss, predict = sess.run([train_op,loss, pred],feed_dict = {pcl_placeholder : data[:,:,:3],
                                           label_placeholder: label,
                                           is_training_pl:True})
            ...
            if ((iter % 3 == 0) and (batch_no == 0)):
                path = "/home/sanket/MS_Thesis/Pointwise-segmentation/saved_model/pointnet2_"
...
if __name__=='__main__':
    ...
    pcl_placeholder, label_placeholder = model.placeholder_inputs(batch_size =2,num_point = 60000)
    ...
    # train(dataset_iterator,num_iteration = 20, loss= loss_model, pred= net_out)
